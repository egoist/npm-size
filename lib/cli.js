        ...
        packages.map((pkg, i) => {
          ...
          return {
            ...
            task: async ({ titles }) => {
              ...
              const { version, size, prettySize } = await check(pkg)
              ...
              titles[i] = exceededSizeLimit
                ? `${padRight(names[i], maxLength)} ${colors.red(
                    prettySize
                  )} ${colors.red(`(size limit: ${bytes.format(sizeLimit)})`)}`
                : `${padRight(names[i], maxLength)} ${colors.green(prettySize)}`
